...
public class OnpeDAO {
	...
	public Object getTotalPresidenciales() {
		db.Sentencia( "SELECT * FROM vTotalVotosPresidenciales");
		return db.getColumna();
	}
	
	public Object getTotalPeru() {
		db.Sentencia( "usp_getVotosPeru");
		return db.getColumna();
	}
	
	public Object getTotalExtranjero() {
		db.Sentencia( "usp_getVotosExtranjero");
		return db.getColumna();
	}
	
	
	
	//Metodo para obterner los datos para el combo
		public Object getZona(int inicio, int fin) {
			db.Sentencia( String.format( "usp_getDepartamentos '%s','%s'", inicio, fin ) );
			return db.getRegistro();
		}
		//METODO PARA OBTENER LA INFORMACION DEL ACTA
		public Object getActas(int nracta) {
			db.Sentencia( String.format( "getActa '%s'", nracta ) );
			return db.getRegistro();
		}
}
